...
int CPVRManager::ReadStream(void* lpBuf, int64_t uiBufSize)
{
  ...
  if (m_currentPlayingChannel)
  {
    ...
    if (m_timeshiftInt)
    {
      ...
      while (m_timeshiftCurrWrapAround+m_pTimeshiftFile->GetPosition()+uiBufSize+131072 > m_TimeshiftReceiver->GetWritten())
      {
      ...
      if (tmp > 0 && tmp < uiBufSize)
        uiBufSize = tmp;

REPEAT_READ:
      bytesReaded = m_pTimeshiftFile->Read(lpBuf, uiBufSize);
      if (bytesReaded <= 0)
      {
        if (CTimeUtils::GetTimeMS() - now > 5*1000)
        {
